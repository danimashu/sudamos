module ApplicationHelper
  ...
  def table_for(collection, *attr_list)
    table_headers = attr_list.map do |attr_name|
      content_tag(:th, collection.name.constantize.human_attribute_name(attr_name))
    end
    ...
    table = content_tag :table, "#{thead} #{tbody}".html_safe,
                        id: collection.name.tableize,
  ...
  def image_url_for(attachment, style)
    if attachment.present?
      attachment.send("#{style}_url".to_sym)
    else
      "missing_#{style}.png"
    end
  end
  ...
  def legal_text
    t("accept", terms: link_to(t("terms"), page_path("terms"), target: "_blank"),
                privacy: link_to(t("privacy"), page_path("privacy"), target: "_blank")).html_safe
  end
  ...
  def contacts_for(advert)
    ...
    button_link_to advert.contacts.count,
                   user_advert_advert_contacts_path(advert),
                   "btn-sm #{klazz}", "envelope"
  end
