...
describe User do
  %w(email password password_confirmation remember_me terms professional
  name state_id phone address company_description provider uid).each do |attr|
  ...
  describe ".find_for_facebook_oauth" do
    ...
    context "when the email exists" do
      user_info = OmniAuth::AuthHash.new info: { email: "daniel@gmail.com" },
    ...
    context "when the email doesn't exist" do
      user_info = OmniAuth::AuthHash.new info: { email: "danielon@gmail.com" },
        extra: { raw_info: { first_name: "Daniel", last_name: "Madrid" } }
    ...
    context "when the email is not valid or blank" do
      user_info = OmniAuth::AuthHash.new info: { email: "invalid" },
        extra: { raw_info: { first_name: "Daniel", last_name: "Madrid" } }
